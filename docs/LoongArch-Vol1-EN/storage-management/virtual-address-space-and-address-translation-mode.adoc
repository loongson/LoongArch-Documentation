...
See <<direct-mapped-address-translation-mode,Direct Mapped Address Translation Mode>> for details on the direct mapped mode and <<storage-management-of-page-table-mapping,Storage Management of Page Table Mapping>> for details on the page table mapped mode.
