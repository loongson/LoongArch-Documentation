...
. Eight general-purpose registers `a0-a7`, and `a0-a1` are also used to return values. Generally, they are used to pass fixed-point arguments and floating-point arguments when the floating-point register is not available . And some other uses will be described below.
Argument bit fields are stored in little endian mode. In addition, the subroutine should ensure that the values of general-purpose registers `s0-s8` and floating-point registers `fs0-fs7` are preserved across procedure calls.

==== Note

In this document, *GRLEN* is the bit width of general-purpose register, *FRLEN* is the bit width of float-point register and *WOA* is the bit width of the argument. General-purpose argument register is denoted as *GAR*, float-point argument register is denoted as *FAR*, and argument register is *AR*(*GAR* or *FAR*).
...
... The structure has one double member, and
