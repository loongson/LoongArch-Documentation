...
`char` is signed, unlike several other recent architectures such as AArch64 or
RISC-V.
Portable code should not use bare `char` at all, but having `char` signed might
help porting legacy x86 or MIPS programs.
It is recommended to always use `signed char` or `unsigned char` explicitly.

Valid values for booleans (`bool`/`_Bool`) are either `0` (`false`) or `1`
(`true`), whether stored in memory or registers.
