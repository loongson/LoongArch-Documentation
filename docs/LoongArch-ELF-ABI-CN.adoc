...
[%header,cols="5*^1"]
|===
|`[31:12]` 位 | `[11:8]` 位 | `[7:6]` 位 | `[5:3]` 位   | `[2:0]` 位

|（保留）  | 操作系统特性 | ABI 版本  | ABI 扩展特性 | 基础 ABI 类型
|===

- `e_flags[7:0]` 标记了 ELF 目标文件中各子程序的 ABI 类型。
...
|`0x1`
|使用 64 位通用寄存器和栈传参，
数据模型为 LP64（`long` 和指针类型宽度为 64 位，`int` 为 32 位）
...
|`0x5`
|使用 32 位通用寄存器和栈传参，
数据模型为 ILP32（`int`，`long` 和指针类型宽度为 32 位）
...
- `e_flags[11:8]` 用于描述 ELF 目标文件所假定的操作系统环境，
如内核 ABI，动态链接等特性。其含义随 `e_ident[EI_OSABI]` 的取值变化，
作为 `e_ident[EI_OSABI]` 的补充。其中各位应默认置 0，除非下表另有说明。

.`e_flags[11:8]` 的含义解释
[%header,cols="5*^1"]
|===
| `e_ident[EI_OSABI]` | 11 位 | 10 位 | 9 位 | 8 位

| `ELFOSABI_NONE`
| 兼容上游 Linux 内核 ABI 时置 1
3*^| （保留）
|===
