...
|（保留）     | ABI 版本  | ABI 扩展特性 | 基础 ABI 类型 (部分)
|===

`e_flags[7:0]` 和 `EI_CLASS` 完整确定了 ELF 目标文件使用的 ABI 类型。
其中，**基础 ABI 类型**由 `e_flags[2:0]` 和 `EI_CLASS` 共同标记，
后者唯一确定了 C 语言整数和指针类型的表示（<<c-lang-data-model, 数据模型>>）和传参方式。
