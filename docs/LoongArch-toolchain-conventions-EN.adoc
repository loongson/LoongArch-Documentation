...
For backwards compatibility, the target tuple environment value `gnuf64` shall
be treated as (canonicalized into) `gnu` if doing so would be feasible and not
bringing any external side effects that goes beyond the local domain-specific
logic (aka "business logic").
(For example: if one must symlink and/or copy files to maintain compatibility
with `gnuf64`, incurring side effects at filesystem / directory layout level,
then the compatibility would be too costly to be "feasible".
A concrete case would be Python whose native extensions' file names contain
exactly one multiarch architecture specifier. In this case it would be
impossible to accommodate v1.00 of this document.)
